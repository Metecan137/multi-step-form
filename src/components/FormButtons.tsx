...
function FormButtons({ onNext }: { onNext?: () => void }) {
    ...
    return (
        <div className={`flex ${buttonNumber > 1 ? "justify-between" : "justify-end"} items-center font-bold`}>
            ...
            <button type={buttonNumber === 1 ? "submit" : "button"} className={`${buttonNumber === 4 ? "bg-primary-purple-600" : "bg-primary-blue-950"} rounded-lg w-30 text-white h-12`}
             onClick={onNext ? onNext : () => dispatch(increaseButtonCount())}>{buttonNumber === 4 ? "Confirm" : "Next Step"}</button>
        </div>
    )
}
